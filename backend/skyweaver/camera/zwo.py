...
class ZwoSdk:
    def __init__(self, library_path: str | None = None) -> None:
        ...
        if not self.library_path:
            raise ZwoSdkUnavailable("ZWO ASI SDK library not found. Install Debian package libasi, or set SKYWEAVER_ZWO_SDK_LIBRARY.")
        try:
    ...
    def find_library() -> str | None:
        ...
        candidates = [
            ...
            *glob.glob("/usr/lib/*-linux-gnu/libASICamera2.so"),
            *glob.glob("/usr/local/lib/*-linux-gnu/libASICamera2.so"),
        ]
...
class ZwoAsiAdapter(CameraAdapter):
    ...
    def _detect_sync(self) -> list[DetectedCamera]:
        ...
        for index in range(sdk.camera_count()):
            cameras.append(self._detected_from_info(sdk.camera_property(index)))
        return cameras
    ...
    def _detected_from_info(self, info: ASICameraInfo) -> DetectedCamera:
        ...
        return DetectedCamera(
            ...
            metadata={
                ...
                "supported_image_types": supported_ints(info.SupportedVideoFormat),
            },
        )
    ...
    async def get_capabilities_for_device(self, device_id: str | None) -> CameraCapabilities:
        try:
            ...
            return CameraCapabilities(
                ...
                cooling=False,
                raw={"sdk_available": False, "install_hint": "Install Debian package libasi, or set SKYWEAVER_ZWO_SDK_LIBRARY."},
            )
        return CameraCapabilities(
            formats=["jpg", "png"],
            controls=["exposure_ms", "gain", "width", "height", "cooling", "target_temperature_c"],
    ...
    def _parse_camera_id(device_id: str | None) -> int | None:
        if not device_id or device_id == "zwo://default":
            return None
        try:
    ...
    def _save_image(data: bytes, path: Path, width: int, height: int, image_type: int, image_format: str) -> None:
        image = Image.frombytes("RGB" if image_type == ASI_IMG_RGB24 else "L", (width, height), data)
