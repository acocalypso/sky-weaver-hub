...
`GET /api/v1/modules` lists built-in and installed modules. The trusted built-in overlay module (`builtin.overlay`) can be enabled and configured with `PATCH /api/v1/modules/builtin.overlay`; custom module uploads stay disabled until sandboxing and signing are implemented. Overlay settings include up to eight text lines, text/background colors, font size, margin, padding, and nine placement options. `GET /api/v1/module-flows` exposes the built-in `post_capture` flow, `PATCH /api/v1/module-flows/builtin.post_capture` can enable/disable it or update its validated built-in module order, and `POST /api/v1/module-flows/builtin.post_capture/run` validates the runnable module list. When the overlay module and post-capture flow are enabled, overlay metadata is recorded on image rows under `metadata.overlay` and `overlay_applied`.
