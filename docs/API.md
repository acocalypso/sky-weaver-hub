...
`GET /api/v1/modules` lists built-in and installed modules. The trusted built-in overlay module (`builtin.overlay`) can be enabled and configured with `PATCH /api/v1/modules/builtin.overlay`; custom module uploads stay disabled until sandboxing and signing are implemented. When enabled, overlay metadata is recorded on image rows under `metadata.overlay` and `overlay_applied`.
