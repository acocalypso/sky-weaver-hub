...
Private image and product download routes under `/api/v1/images/{image_id}/download` and `/api/v1/products/{product_id}/download` require `read:images`. Public download routes remain unauthenticated only under `/api/v1/public/...` and still honor the public-page enabled setting.
