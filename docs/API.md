...
`PATCH /api/v1/settings` validates the known settings groups exposed by the admin UI: `observatory`, `storage`, `public_page`, and `security`. Patches for those groups are merged with existing values before validation so partial group updates remain supported. Invalid values return `422`; non-object setting groups return `400`. Unknown extension groups are allowed only as JSON objects so migration/plugin metadata can remain additive without bypassing basic shape checks.
...
Remote upload endpoints require authenticated admin or processing scopes. Implemented target types are `filesystem`, `rsync_ssh`, `scp_ssh`, `sftp_ssh`, `ftp`, and `ftps`. SSH-based targets use key/agent-based SSH only and store no password. FTP/FTPS targets require a password. Remote target configs are stored as a local Fernet encrypted envelope derived from `SKYWEAVER_SECRET_KEY`, and secret values are redacted from API responses, diagnostics, and logs. Changing `SKYWEAVER_SECRET_KEY` without migrating secrets will make existing encrypted target configs unreadable.
...
Allsky migration endpoints require admin scope. Detection and preview do not modify source data. Import runs through the processing worker, copies recognized capture images, dark frames, products, and overlay image assets into Sky Weaver-owned storage while excluding Allsky web/documentation/config/overlay asset trees from gallery imports, records provenance under `metadata.migration`, updates processing progress, returns a compact `import_log`, and applies a conservative subset of settings, day/night profile settings, processing/public/storage settings, and basic overlay text settings with a rollback snapshot. Rollback only removes Sky Weaver-created rows/files and restores settings for the selected import job:
...
`GET /api/v1/system/services` returns Sky Weaver service rows, including capture daemon and processing worker heartbeat, PID, last claimed job, and last success timestamps when those processes have reported activity. `GET /api/v1/system/services/{name}` returns per-service `systemctl show` properties, recent `journalctl` output, a `failure_analysis` summary, and `unit_history` timestamps/restart metadata when systemd tooling is available. These fields are read-only diagnostics for operators and mobile clients; service actions remain separate admin-scoped `POST /api/v1/system/services/{name}/{action}` calls.

Private image and product download routes under `/api/v1/images/{image_id}/download`, `/api/v1/images/{image_id}/thumbnail`, and `/api/v1/products/{product_id}/download` require `read:images`. Public download routes remain unauthenticated only under `/api/v1/public/...` and still honor the public-page enabled setting.
