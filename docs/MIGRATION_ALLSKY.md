...
Current import support copies Allsky images into Sky Weaver image storage and imports keograms, startrails, and timelapses as generated night products. It also imports a conservative subset of settings when detected: observatory name/location/timezone, schedule sun angle, public page intent, and camera hints. Every imported row records migration provenance in metadata, including the import job id and original file path. Rollback deletes only Sky Weaver-created rows and copied files for that import job and restores the previous Sky Weaver settings snapshot.
