...
The product is not yet Allsky feature-complete. The main missing areas are longer real outdoor overnight soak validation, richer image-product options, additional network remote upload protocols, full Allsky settings/dark-frame/overlay import, dark-frame processing, and broader validated camera adapter coverage beyond Raspberry Pi libcamera hardware.
...
- Expand real import beyond the currently implemented images, timelapses, keograms, startrails, selected observatory/schedule/public-page settings, and camera hints:
  - dark frames
  - additional settings
...
- Remote upload supports filesystem and rsync-over-SSH targets. SFTP/SCP/FTP targets are not implemented.
- Allsky migration imports images, keograms, startrails, timelapses, selected settings, and camera hints by copying files into Sky Weaver storage and applying a restorable settings snapshot. Dark frames, broad settings translation, and overlay assets are not imported yet.
...
- `backend\\.venv\\Scripts\\python -m pytest -p no:cacheprovider --basetemp .tmp\\pytest-all backend\\tests`: passed with 74 tests.
